...
| 運営: 告知する | 3 媒体への告知を投稿する（日程未確定なら押せない。詳細は後述） |
...
ポストの「📣 運営: 告知する」ボタンから、**X・Discord お知らせチャンネル・VRChat グループ掲示板**の 3 媒体へ投稿します。
週次の事前告知・開催告知とは**完全に独立した投稿**で、`/pre-announce` や `/post-announcement` とは連動しません。
...
| 媒体 | 内容 | 冪等キー |
| :--- | :--- | :--- |
| X | `src/x/assets/lt_announce_template.txt` の文面＋告知画像 | ツイート ID |
...
- X は日本語 1 文字を 2 とカウントする 280 上限があるため、超える場合は**タイトルだけを縮めて**収めます
  （日付・登壇者・ハッシュタグは残します）。
...
| `LT_ANNOUNCE_ROLE_ID` | LT 告知でメンションするロール ID（任意、未設定ならメンションなし） |
